...
class GMM(object):
    ...
    def softmax(self, logit):  # [5pts]
        ...
        softmax = np.exp(np.subtract(logit, self.logsumexp(logit)))
    ...
    def logsumexp(self, logit):  # [5pts]
        ...
        """
        max_logit_dims = np.max(logit, axis=1)
        logit_stable = np.subtract(logit, max_logit_dims[:,None])
        logsumexp_stable = (np.log(np.sum(np.exp(logit_stable),axis=1)+LOG_CONST)+max_logit_dims).reshape((logit.shape[0],1))
    ...
    def multinormalPDF(self, points, mu_i, sigma_i):  # [5pts]
        ...
        points_centered = np.subtract(points,mu_i)
        try:
            A = np.dot(points_centered,np.linalg.inv(sigma_i))
        except np.linalg.LinAlgError:
            A = np.dot(points_centered,np.linalg.inv(sigma_i+SIGMA_CONST))

        B = np.multiply(A.T,points_centered.T).sum(axis=0)
        normal_pdf = 1/np.sqrt((2*np.pi)**len(mu_i) * np.linalg.det(sigma_i)) * np.exp(-B/2) 
    ...
    def _init_components(self, **kwargs):  # [5pts]
        ...
        sigma = np.stack([np.eye(self.D) for _ in range(self.K)],axis=0)
    ...
    def _ll_joint(self, pi, mu, sigma, full_matrix=FULL_MATRIX, **kwargs):  # [10 pts]
        ...
        ll = np.stack([np.log(pi[k] + LOG_CONST) + np.log(self.multinormalPDF(self.points,mu[k,:],sigma[k,:,:])+LOG_CONST) for k in range(self.K)], axis=1)
    ...
    def _M_step(self, gamma, full_matrix=FULL_MATRIX, **kwargs):  # [10pts]
        ...
        #if full_matrix is True:
        pi = gamma.sum(axis=0) / self.N
        mu = np.divide(np.dot(gamma.T, self.points), gamma.sum(axis=0).reshape((-1,1)))
        sigma = np.stack([np.dot(np.multiply(gamma[:,k].T,np.subtract(self.points,mu[k]).T),np.subtract(self.points,mu[k])) / gamma[:,k].sum(0) for k in range(self.K)], axis=0)
